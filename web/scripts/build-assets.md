...
- All four kit files' format + exact dimensions are pinned by `web/test/og-image.test.ts`
  (the mint fields are immutable, so these facts are test-asserted, not eyeballed).
...
Regenerate: `cd web && node scripts/build-og.mjs` (needs the e2e browser install,
`npx playwright install`). Deterministic per machine — same repo state, same card layout;
