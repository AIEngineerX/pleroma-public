...
export default function WalletButton({ onConnect }: { onConnect: (w: WalletHandle) => void }) {
  ...
  return (
    <div className="font-machine text-sm">
      <button className="min-h-11 px-3 underline text-ink-faded"
        onClick={async () => { setWallets(await availableWallets()); setOpen(true); }}>{copy.connect}</button>
      {open && (
        <ul className="mt-2 border" style={{ borderColor: "var(--color-ground-aged)" }}>
          {wallets.length === 0 && <li className="p-3 text-ink-faded">no wallet found; you may still {copy.offerAnon.toLowerCase()}</li>}
          {wallets.map(w => (
            <li key={w.name}>
