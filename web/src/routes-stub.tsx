      ...
      <>
        <main {...bindHold} className="banding min-h-screen mx-auto px-6 md:grid md:grid-cols-[60fr_40fr_4rem] md:grid-rows-[55vh_auto] md:gap-8"
          ...
          <section aria-label="the page" className="relative min-h-[40vh] sticky top-0 md:relative md:col-start-1 md:row-start-1 md:h-[55vh] flex flex-col items-center justify-center gap-6">
            {/* rite mode (the single allowed candle-glow) only from offertory_close through sermon
                (rite.candleDark); "scheduled" is still daylight, so the Stain stays dormant then too. */}
            <Stain state={rite.candleDark ? "rite" : "dormant"} pigment={[0.55, 0.20, 0.32]} amplitude={amplitude} onSim={setStainSim} />
            <Emblem />
          ...
          {/* the market rail (Task 11): every money element is built ONLY from state.mint (Task 1
              ...
              until the Worker flips phase to live; before launch there is nothing here ("no heart
              yet"). Same column as the Reliquary so it stacks beneath it on both desktop and mobile. */}
          {state?.phase === "live" && state.mint && (
