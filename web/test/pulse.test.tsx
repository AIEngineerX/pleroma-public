...
describe("Pulse — the always-visible home for PULSE, independent of the mint-gated market section", () => {
  ...
  it("sweeps the trace at the real bpm — not a generic cadence, the actual current rate", () => {
    const fed = render({ kind: "current", value: { state: "fed", buys: 0, sells: 0, holders: 0 }, receivedAt: 1 });
    expect(fed).toContain("data-pulse-trace");
    // fed = 54 bpm -> 60/54 = 1.111s per sweep.
    expect(fed).toContain("--pulse-duration:1.111s");
  });

  it("tints the trace with the same pigment law the ink itself uses, not an arbitrary color", () => {
  ...
  it("draws only the flat baseline before the first vitals response — no beat, no trace to sweep", () => {
    const html = render({ kind: "unknown" });
