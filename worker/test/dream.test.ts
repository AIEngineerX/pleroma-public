...
describe("state-driven compose — a rite completing after its 03:00 run still gets its night", () => {
  ...
  it("selects recent completed rites with kept relics and no dream, oldest-first, skipping the rest", async () => {
    ...
    // Not eligible: complete but kept nothing.
    ...
    expect(await composableRiteDates(env, now)).toEqual(["2026-08-01", "2026-08-02"]);
  });
